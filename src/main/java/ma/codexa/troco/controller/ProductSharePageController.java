...
public class ProductSharePageController {
    ...
    @Value("${app.site.public-url:https://troco.com}")
    private String publicSiteUrl;

    @Value("${app.site.public-name:Troco}")
    ...
    private String buildDescription(Product product) {
        String price = NumberFormat.getInstance(Locale.FRANCE).format(Math.round(product.getPrice())) + " DH";
        String desc = product.getDescription() != null ? product.getDescription().trim() : "";
        if (desc.length() > 160) {
            desc = desc.substring(0, 157) + "…";
        }
        if (desc.isEmpty()) {
            desc = "Emballage professionnel Troco — " + price;
        } else {
            desc = desc + " — " + price;
        }
