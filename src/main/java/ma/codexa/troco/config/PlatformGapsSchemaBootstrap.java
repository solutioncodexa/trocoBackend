    ...
    private void bootstrapStoreSettingsAndOrders() {
        try {
            String[] cols = {
                    ...
                    "payment_paypal_enabled BOOLEAN DEFAULT FALSE",
                    "paypal_client_id VARCHAR(255)",
                    "paypal_client_secret VARCHAR(255)",
                    "paypal_mode VARCHAR(20) DEFAULT 'sandbox'",
                    "paypal_verified_at TIMESTAMP",
                    "cmi_client_id VARCHAR(120)",
                    "cmi_store_key VARCHAR(255)",
                    "cmi_verified_at TIMESTAMP",
        ...
        try {
            ...
            jdbc.execute("""
                    CREATE TABLE IF NOT EXISTS abandoned_carts (
                        ...
                        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                        CONSTRAINT uq_abandoned_cart_token UNIQUE (recovery_token),
                        CONSTRAINT uq_abandoned_cart_session UNIQUE (fournisseur_id, session_key)
                    )
                    """);
            jdbc.execute("""
